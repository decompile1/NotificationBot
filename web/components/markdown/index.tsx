...
export default async function CustomMarkdown({ markdown }: { markdown: string; }) {
    ...
    async function parseDiscordMarkdown(content: string) {
        ...
        return content
            ...
            .replace(/<(@&)\d{15,21}>/g, () => {
                return renderToString(<User username="test-role" />);
            })
            .replace(/<(#!?)\d{15,21}>/g, () => {
                return renderToString(<Channel name="test-channel" />);
            })
    ...
    return (
        <ReactMarkdown
            ...
            components={{
                ...
                strong: (props) => <span className="font-extrabold dark:text-red-700 text-red-900" {...props} />,
