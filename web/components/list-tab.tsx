    ...
    return (
        <div className="mt-2 mb-4 flex items-center relative">
            ...
            >
                <TabsList
                    ref={ref}
                    className="bg-inherit border-b-2 border-red-800 p-0 w-full justify-start rounded-none overflow-y-auto overflow-x-auto scrollbar-hide"
                >
                    {tabs.map((tab) => (
                        <TabsTrigger
                            key={tab.value}
                            value={tab.value}
                            className='data-[state=active]:border-red-500 hover:text-foreground h-full rounded-b-none border-b-2 border-transparent flex gap-2 whitespace-nowrap'
                            disabled={disabled}
                        >
                            {tab.icon}
                            {tab.name}
