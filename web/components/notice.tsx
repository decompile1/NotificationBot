...
export default function Notice({ className, message, type, location = "side", children }: Props) {
    return (
        <div
            className={cn(
        ...
        >
            <div className="flex items-center gap-2">
                {type === NoticeType.Info ? <HiExclamationCircle className="size-4" /> : <HiExclamation className="size-4" />}
                <div className="text-medium">{message}</div>
            </div>

            {children && <div className={cn(location === "side" && "ml-auto")}>{children}</div>}
        </div>
    );
}