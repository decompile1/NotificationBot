...
import DiscordAppBadge from "./appbadge";
...
export default function DiscordMessage({ children, commandUsed, user, mode }: Props) {
    return (
        <div
            className={cn("group relative rounded-lg px-1 w-full", mode === "DARK" ? "text-neutral-100" : "text-neutral-900")}
        >
            {commandUsed ? (
                <div
                    className={cn(
                        "flex items-center gap-1 opacity-70 text-sm ml-4",
                ...
                >
                    ...
                    <div className="mx-0.5 flex items-center gap-1 font-semibold whitespace-nowrap overflow-hidden text-ellipsis shrink-0">
                        ...
                        />
                        <span className={mode === "DARK" ? "text-red-400" : "text-red-600"}>{commandUsed.username}</span>
                        {commandUsed.bot && <DiscordAppBadge />}
                        used
            ...
            <div className="flex flex-row items-start pointer-events-none [&>*]:pointer-events-auto">
                ...
                <div className="flex-1 w-0">
                    <div className="flex items-center space-x-2">
                        ...
                        {user.bot && <DiscordAppBadge />}
                        <time className="mt-[2px] text-xs opacity-50 truncate" dateTime={new Date(1019060317).toISOString()}>
