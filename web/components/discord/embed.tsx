...
export default function DiscordMessageEmbed({
    ...
    !image &&
    !footer?.text &&
    (!children || children.toString() === ",false" || (Array.isArray(children) && !children.filter(Boolean).length))
    )
        return <></>;
                ...
                <div className={thumbnail ? "w-9/12" : "w-full"}>
                    {author && (
                        ...
                        >
                            ...
                            <DiscordMarkdown mode={mode} text={author.text} embed={true} />
                        </div>
                    )}
                    {title && (
                        <div
                            className={cn(mode === "DARK" ? "text-neutral-100" : "text-neutral-900", "font-semibold text-lg mb-2")}
                        >
                            <DiscordMarkdown mode={mode} text={title} embed={true} />
                        </div>
                    )}
            ...
            {footer?.text && (
                <div className="flex gap-1 items-center mt-3">
                    ...
                    <span className="text-xs">
                        <DiscordMarkdown mode={mode} text={footer.text} embed={true} />
                    </span>
                </div>
            )}
        </div>
...
function replaceTemplatesToUrl(input: string) {
    ...
    if (/^{(video|stream)\.thumbnail}/.test(input))
        return "/_next/image?url=/notifications-thumbnail-placeholder.webp&w=384&q=75";
